...
### Phase 2 — 오프라인 검색 품질 검증 (PC/Colab) ★ 게이트 ← **지금 여기**
**여기서 recall@k가 안 나오면 앱 작업은 시작하지 않는다.**
- 후보 임베더 2~3종 비교(영어 단일 언어 모델 중에서). 지표: **Recall@1/3/5, MRR**, 질문 유형별로 나눠 볼 것
- 개선책 벤치: `query:`/`passage:` 프리픽스(e5 계열), 임베딩 텍스트 포맷(§5-2 #1), 타입별 k 배분,
