...
⚠️ **아래 Phase 본문은 2026-07-17에 쓴 원안이고, 체크박스는 그때 이후 갱신하지 않았다.**
실제로 어디까지 왔는지는 이 표와 `CLAUDE.md`가 최신이다 — **본문과 어긋나면 이 표를 믿을 것.**

| 원안 Phase | 실제 상태 | 어디를 볼 것 |
|---|---|---|
| 0 기준선 | ✅ 스키마·프롬프트셋 확정, 벤치 자동화까지 완료 | `CLAUDE.md` "현재 측정 중인 성능 지표" |
| 1 LiteRT-LM 이전 | ✅ **완료.** 두 엔진이 `InferenceEngine` 추상화 위에 공존, 측정 끝 | `notes/2026-07-18-inference-engine-abstraction.md` |
| 2 하드웨어 최적화 | 🟡 **원인 규명 완료, 해법은 업스트림에 막힘.** 우리 변환본은 활성값 fp32라 GPU가 CPU로 강등된다 | `notes/2026-08-01-gpu-fallback-activation-type.md` |
| 3 LoRA | 🟡 **학습·변환 완료, 평가 진행 중.** 한국어 화법은 전이됐으나 영어로 재면 안 나와서 학습셋을 영어로 재작성해 둠(재학습 전) | `CLAUDE.md` "생성 길이와 응답 수집" |
| 4 RAG | 🟡 **Phase 1(코퍼스·골드셋) 완료, Phase 2(임베더 선정) 진행 중.** ⚠️ 원안과 설계가 달라졌다 — 작가·화파는 검색이 아니라 **조인**으로 처리한다 | **`rag-roadmap.md`** (이게 RAG의 단일 출처), `notes/2026-08-15-phase2-retrieval.md` |
| 5 멀티에이전트 자동화 | 🟡 부분 — adb 무인 실행·냉각 게이트·조건 기록은 돌아간다. 에이전트 오케스트레이션은 미착수 | `scripts/run-benchmarks.ps1` |

**원안과 실제가 가장 크게 갈린 곳**: Phase 0의 "RAG 정답셋 30~50건"은 만들었지만(`datasets/rag_goldset.jsonl` 40문항), 코퍼스가 원안의 "작품 레코드"가 아니라 **외부 수집 배경지식**으로 바뀌었다(rag-roadmap 결정 5). 눈앞 작품 정보는 이미 정확한 key lookup으로 들어가므로 근사 검색으로 대체하면 퇴보라서다.
